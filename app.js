...
app.get('/delete/:id', async (req, res) => {
  ...
  try {
    const [row] = await connection.promise().query('SELECT imagen FROM contactos WHERE id = ?', [id]);
    ...
    await connection.promise().query('DELETE FROM contactos WHERE id = ?', [id]);
    res.redirect('/');
  } catch (error) {
...
app.post('/save', upload.single('imagen'), async (req, res) => {
  ...
  try {
    ...
    await connection.promise().query(
      ...
      { nombre, apellidos, direccion, correo, edad, imagen: s3Key }
    );
    res.redirect('/');
  } catch (error) {
...
app.get('/edit/:id', async (req, res) => {
  ...
  try {
    const [row] = await connection.promise().query('SELECT * FROM contactos WHERE id = ?', [id]);
    const contactos = row[0];

    if (contactos && contactos.imagen) {
      const imageUrl = `/images/${contactos.imagen}`; // URL de la imagen
      res.render('edit', { contactos: contactos, imageUrl: imageUrl });
    } else {
...
app.post('/update', upload.single('imagen'), async (req, res) => {
  ...
  try {
    await connection.promise().query(
      'UPDATE contactos SET nombre = ?, apellidos = ?, direccion = ?, correo = ?, edad = ?, imagen = ? WHERE id = ?',
      [nombre, apellidos, direccion, correo, edad, imagen, id]
    );
    res.redirect('/');
  } catch (error) {
...
app.get('/delete-all', async (req, res) => {
  try {
    const [rows] = await connection.promise().query('SELECT imagen FROM contactos');
    ...
    await connection.promise().query('DELETE FROM contactos');
